...
Uses the Apple Silicon accelerometer (Bosch BMI286 IMU via IOKit HID) to detect physical hits on your laptop and plays audio responses. On non-Apple-Silicon platforms (including Windows), it runs in manual trigger mode (press Enter to simulate a slap).

## Requirements

- Apple Silicon macOS (M2+) for automatic accelerometer detection
- On non-Apple-Silicon platforms (including Windows), manual trigger mode is available
- `sudo` is only required on Apple Silicon macOS
...
# Windows / non-Apple-Silicon mode — manual trigger
spank --trigger both
# Enter or any key triggers sounds, q quits
...
**Trigger mode** (`--trigger`, non-Apple-Silicon only):

- `enter`: trigger when you press Enter
- `key`: trigger on any key press (instant, no Enter needed)
- `both` (default): Enter and key press triggers are both enabled
